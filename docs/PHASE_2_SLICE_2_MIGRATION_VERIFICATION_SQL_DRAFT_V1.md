...
* Status: Draft — pending high-risk review
* Phase: 2
* Slice: 2
* Artifact type: Migration Verification SQL Draft
* Source migration: committed
* Verification SQL file creation: not authorized
...
This document contains a review-only SQL draft that could later verify the committed Phase 2 Slice 2 deletion-request-governance migration after a separately authorized DEV apply:

`supabase/migrations/20260719120000_phase_2_slice_2_deletion_request_governance.sql`

The draft implements exact post-apply catalog, privilege, RLS, transition-guard, and deferred-consistency checks derived from the committed migration and the accepted verification plan.
...
Inspected source artifacts (read completely before authoring):
...
7. `docs/PHASE_2_SLICE_2_MIGRATION_VERIFICATION_PLAN.md`
8. `supabase/migrations/20260719120000_phase_2_slice_2_deletion_request_governance.sql`

**Repository baseline (Git identity):**

`73af733400efd68a6657bbbc11c52814e6c9d3f2`

This is a Git repository identity for the local and `origin/main` HEAD at authoring time. It is **not** a remote Supabase migration-history identity. No claim is made that the migration has been applied to any Supabase project.

---

## Verification Architecture

Verification is split into two separately gated SQL blocks.

### Block A — Read-Only Catalog and Privilege Verification

Purpose:

* exact post-apply catalog verification
* RLS and privilege verification
* no data mutation
* no test-data insertion

Block A runs inside an explicit `READ ONLY` transaction and finishes with `COMMIT` without mutating durable data.

### Block B — Transaction-Rolled-Back Behavioural Verification

Purpose:

* transition-guard verification
* deferred cross-table consistency verification
* temporary verification rows only
* complete rollback
* separately authorized execution only

Block B is **not** read-only even though it must end in `ROLLBACK`. It performs temporary DML against only the two Slice 2 governance tables, then discards all changes.

---

## Operator Preconditions

Future execution (not authorized by this document) requires all of the following:

* DEV environment only
* separately authorized migration apply completed successfully
* exact expected migration identity confirmed (repository SHA plus applied history identity once available)
* approved connection context
* operator understands that Block B performs temporary DML
* no concurrent workflow may use the synthetic test identifiers defined in Block B
* statement and lock timeouts must be set locally inside Block B
* evidence capture is prepared (result grids retained; secrets redacted)
* PROD execution remains prohibited
* **Block B execution-role contract (fail-closed):** Block B must run only under an approved privileged DEV verification session (table owner / postgres-equivalent) capable of exercising INSERT, UPDATE, and DELETE directly on `public.deletion_requests` and `public.deletion_request_executions`, including protected columns required by negative tests. Block B is **not** a realistic `service_role` runtime test and is **not** proof of `service_role` least-privilege behaviour; that posture is verified by Block A catalog ACL checks. This draft does not use `SET ROLE`, JWT claim impersonation, or `auth.users` mutation.
* **Pre-existing-object baseline:** proving that non-Slice-2 objects were unchanged requires a separate pre-apply versus post-apply catalog baseline artifact retained in the DEV evidence package. Block A final catalogs alone cannot prove migration ordering or unchanged pre-existing objects. Absence of that external baseline must be reported as a gate defect during DEV evidence review.

This document contains no secrets, passwords, tokens, or connection strings.
...
* Any **P0** or **P1** `FAIL` blocks progression toward PROD consideration.
* No automatic repair is authorized by this script or document.
* Catalog mismatch is not repaired by this script.
* Behavioural failure requires investigation and a new reviewed correction migration if a defect is confirmed — not ad-hoc DDL.
* `INFO` does not override `FAIL`.
* Unexpected `NOT_RUN` fails Block B. Only explicitly allowed precondition-dependent `NOT_RUN` (hard `B-00` object/role failure) or seed-dependency `NOT_RUN` (`B-02-00` / `B-03-00` / `B-04-07` when the seed case failed) may be tolerated; hard precondition failure cannot yield Block B `PASS`.
* SQLSTATE `42501` is never an expected guard rejection.
* Block B rollback must be independently confirmed via the real post-`ROLLBACK` residue statement (`B-09-04`) — not commented, not optional.
* PROD remains prohibited regardless of a DEV `PASS` until a separate decision.

## Coverage Counts (recalculated against this draft)

| Metric | Count |
|--------|------:|
| Block A unique check IDs (incl. `A-12-01`) | 74 |
| Block A rows in `all_a` (excl. summary) | 73 |
| Block A binding non-INFO checks in `all_a` | 68 |
| Block A INFO checks in `all_a` | 5 |
| Block B unique case IDs referenced in SQL (incl. summary + residue) | 60 |
| Block B unique case IDs in temp results (happy path, excl. summary/residue/scaffolds) | 56 |
| Block B substantive behavioural cases (`B-01`–`B-08`, excl. `B-0x-00` scaffolds) | 51 |
| Expected-success behavioural cases | 10 |
| Expected-rejection behavioural cases | 40 |
| Block B INFO cases in temp results (`B-07-03`, `B-09-01`…`B-09-03`) | 4 |
| Block B summary rows (`B-10-01`, emitted once outside temp table) | 1 |
| Post-rollback residue rows (`B-09-04`, executable after `ROLLBACK`) | 1 |
| Conditional NOT_RUN scaffolds | `B-02-00`, `B-03-00`, `B-04-07` (seed dependency); bulk NOT_RUN when `B-00` fails |

Counts are derived from the SQL in this document after correction. Do not preserve stale counts from earlier drafts.

---

## Design-to-Verification Traceability

| Migration requirement | Source migration object or rule | Block A check IDs | Block B case IDs | Expected severity | Evidence produced |
|-----------------------|---------------------------------|-------------------|------------------|-------------------|-------------------|
| Table structure | `CREATE TABLE` both governance tables; column defaults | A-02-01…A-02-05 | — | P1 | Column inventory PASS/FAIL rows |
| Vocabularies | Named CHECKs for scope/state/resolution | A-03-01…A-03-06 | B-01-02, B-01-17, B-04-04 | P1 | Catalog expression checks + behavioural rejection |
| Scope/target matrix | `deletion_requests_scope_target_matrix_check` | A-03-08 | B-01-06…B-01-08, B-01-12…B-01-16 | P1 | CHECK rejection cases |
| Evidence whitelist / nonempty email | named CHECKs | A-03-03, A-03-09 | B-01-09…B-01-11 | P1 | Catalog + behavioural |
| State/resolution coupling | `deletion_requests_state_resolution_coupling_check` + guard | A-03-07 | B-04-04 | P1 | Coupling enforcement |
| Timestamp ordering | `deletion_requests_timestamp_ordering_check` | A-03-10 | B-04-06 | P1 | Ordering rejection |
| Executed requires validation | CHECK + transition-guard executed branch | A-03-13, A-06-02 | B-04-02, B-03-01 | P1 | Guard/CHECK evidence |
| Transition guard | `enforce_deletion_request_transition` + `trg_deletion_requests_transition_guard` | A-05-*, A-06-02 | B-01…B-04 | P0/P1 | Function/trigger + behavioural matrix |
| Immutable fields | Guard UPDATE immutability branch | A-06-02 | B-02-01…B-02-08 | P1 | Per-field rejection |
| Deferred consistency | `enforce_deletion_request_execution_consistency` + both deferred triggers | A-05-*, A-06-03, A-06-04 | B-05…B-08 | P0/P1 | Per-case subtransaction isolation + IMMEDIATE checks |
| RLS | `ENABLE ROW LEVEL SECURITY` + ownership SELECT policy | A-07-01…A-07-03 (binding); A-07-04 INFO exact-text | — | P0/P2 | Policy catalog rows |
| Grants | REVOKE/GRANT matrix; table ACL via `aclexplode`; column grants separate | A-08-*, A-09-*, A-11-04 | — | P0 | Table ACL ≠ column privilege |
| Function security | `SECURITY INVOKER`; EXECUTE hardening | A-05-03, A-05-04, A-11-* | — | P0 | `prosecdef` + EXECUTE matrix |
| Dormant-on-arrival posture | No migration DML; empty tables post-apply | **A-01-06** (empty-table counts); comments A-10-* | B-09-04 residue | P1/P0 | A-01 object existence does **not** prove emptiness |
| Pre-existing objects unchanged | External pre-apply vs post-apply catalog baseline artifact | A-11-07 INFO + DEV evidence review | — | P0 gate | Block A alone cannot prove unchanged pre-existing objects |
| F-P2-3 | Accepted residual; not redesigned | — | B-07-03 (INFO), B-10-01 | P2 INFO | Residual observation row; not temporal freezing |
...
* A later `service_role` attribution insert may remain possible for an already `executed` `account_wide` request while all other invariants hold (parent remains `executed`, scope remains `account_wide`, uniqueness of `(deletion_request_id, scan_record_id)` satisfied).
* The workflow must create all account-wide attribution rows inside the original atomic execution transaction.
* Database-level temporal freezing is outside this Slice.
* The verification SQL reports this behaviour (`B-07-03` as `INFO`) and does **not** redesign or classify the accepted posture as failure when the migration behaves as designed.

---

## Known Limitations

* Catalog normalization differences may affect raw `pg_get_constraintdef` / `pg_get_expr` text (casts; `IN` vs `= ANY (ARRAY[])`; whitespace). Block A CHECK verification uses normalized semantic matchers with exact closed-set literal equality for scope/state/resolution/whitelist vocabularies; policy binding check (`A-07-03`) requires equality ownership predicate via exact or harmlessly normalized equality forms only (token-only semantic fallback prohibited); exact textual policy form is `A-07-04` INFO only.
* Function/table owner identity depends on the future DEV apply role and is reported as `INFO`, not a hard role-name FAIL.
* Object catalogs alone cannot prove migration ordering or Supabase migration-history identity.
* **Pre-existing-object unchanged verification** requires a separate pre-apply versus post-apply catalog baseline artifact. Block A alone cannot prove no pre-existing object changed. Absence of that external baseline must be reported as a DEV evidence-review gate defect (`A-11-07` documents the requirement). This draft does not create persistent baseline tables or helper objects.
* Table-level privilege absence is proven via `pg_catalog.aclexplode` over relation ACL entries (not `has_table_privilege` for INSERT/UPDATE). Column-level authorization remains in A-09. Approved column grants must not be treated as table-level grants. Direct `aclexplode` relation-ACL inspection verifies direct table ACL entries; effective privileges inherited through role membership require separate role-membership evidence during DEV review.
* Block A privilege catalog checks are not equivalent to runtime JWT / `service_role` session proof.
* Block B is a privileged DEV verification session test (owner/postgres-equivalent), not a `service_role` realism test. Hard precondition `B-00-02` fail-closes when required INSERT/UPDATE/DELETE (including protected columns) is unavailable. No `SET ROLE`, JWT impersonation, or `auth.users` mutation.
* Behaviour cases require explicit writable-test authorization separate from Block A.
* Rollback evidence requires the real post-rollback residue statement (`B-09-04`) after `ROLLBACK;`.
* Catalog-only rules that are intentionally not behaviourally repeated beyond the expanded B-01 matrix (for example every timestamp-edge permutation) remain catalog-bound via A-03 expression checks; traceability must not claim behavioural coverage that is absent.
* These limitations do not weaken required checks: required failures remain `FAIL`.

---

## Non-Execution Boundary

* This document has not been executed.
* No verification SQL file has been created under `supabase/verification/`.
* No SQL Editor execution is authorized.
* No Supabase CLI execution is authorized.
* No DEV or PROD change is authorized.
* Block B test-data insertion is prohibited until a separate behavioural-test gate.
* This document may only be reviewed and corrected.
* A later gate is required before creating an execution candidate under `supabase/verification/`.

---

## Current Decision

The SQL draft is ready only for read-only high-risk review.

This document does **not** authorize:

* file creation under `supabase/verification/`
* DEV execution
* PROD execution
* test-data insertion
...
*End of Phase 2 Slice 2 Migration Verification SQL Draft V1.*
